...
document.getElementById('ai-continue').addEventListener('click', async (e) => {
  ...
  if (!keys().openai) {
    err.textContent = 'Your OpenAI API key is required';
    return;
  }
  btn.disabled = true;
  try {
    ...
    if (res.status === 401) return (location.href = '/login');
    const body = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(body.error || 'Could not suggest categories');
...
async function showCategoriesStep(body) {
  ...
  if (body.categories && body.categories.length) {
    ...
    // No categories to tick, so there is nothing to skip past either.
    renderSuggestions([], body.note || 'No categories could be suggested from your transactions.');
